...
A new collection type to store submissions to the _All Star_ project has been set up. An entry of this collection features the same content fields as entries of the ootibOO! submission collection.
...
---
### Borken project card badge layout

During an partial automatic re-build of the project overview page (play.ootiboo.com/projects) triggered by a re-ordering of the projects, the CSS classnames responsible for aligning the CU and NC badge elements displayed on project cards got mixed up by the server, causing badges on two card elements to be in the wrong position and be rendered too small + with the wrong background. 
